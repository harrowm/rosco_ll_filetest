// File to test reading and writing to a SD Cards on the rosco_m68k without using the sdfat library
// Malcolm Harrow November 2023
// MIT license

// This program can be compile with:
//     ROSCO_M68K_DIR=~/rosco_m68k make EXTRA_CFLAGS=-DROSCO=1
//
// ***** NOTE THAT RUNNING THIS PROGRAM WILL DESTROY THE FAT32 PARTITION ON THE SD CARD *****
...
#define DATASIZE 512 // Now we are dealing with sectors on FAT32, this will always be 512 ..
#define TIMES_TO_WRITE 10000
#define STARTSECTOR 0x3000  // the fat32 partition will get trashed ..
...
int kmain() {
    ...
    printf("Running test for sector size %d and %d times\n", DATASIZE, TIMES_TO_WRITE);
    ...
    for (int i=0; i<TIMES_TO_WRITE; i++) {
        ...
        if (!SD_write_block(&sd, (uint32_t) (i+STARTSECTOR), dataset)) {
            printf("Failed to write sector %08x\n", (i+STARTSECTOR));
            continue;
        }

        if (!SD_read_block(&sd, (uint32_t) (i+STARTSECTOR), dataset)) {
            printf("Failed to read sector %08x\n", (i+STARTSECTOR));
            continue;
        }
        ...
        if (c != c2) {
            printf("Read completed but checksums are different for sector %08x\n", (i+STARTSECTOR));
...
#else
int main() {
    printf("This program will destroy the FAT32 structure of the SD card its run against.\n");
    printf("You must compile for the rosco_m68k, it cant be tested on a Mac or PC.\n");
